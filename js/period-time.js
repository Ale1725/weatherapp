...
export function periodTimeTemplate({ temp, date, icon, descripcion}) {
  return `
    <li class="dayWeather-item is-selected">
      <span class="dayWeather-time">${date}</span>
      <img class="dayWeather-icon" height="48" width="48" src="https://openweathermap.org/img/wn/${icon}@2x.png" alt="${descripcion}" rain="">
...
export function createPeriodTime(weather) {
   ...
   // description
    ...
    const config = {
     ...
     descripcion: weather.weather[0].descripcion
   }
